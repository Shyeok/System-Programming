...
typedef struct bplink // breakpoint를 저장할 자료구조
{
    int addr;
    struct bplink *nextbp;
}bpcell;

typedef struct esymlink // ESTAB를 위한 자료구조
{
    char esym[10];
    int locv;
    int length;
    int typeflag;
    struct esymlink *nextesym;
}esymcell;
...
void printloadmap(); // loadmap를 출력해준다.
